...
/**
 * GuidelinesLibrary (single-file starter)
 * --------------------------------------
 * What this does:
 * - Displays guideline images (PNG/JPG/WebP) in a fast, responsive grid
 * - Search by title/keywords; filter by category; quick favorites
 * - Modal viewer with zoom, pan, rotate, next/prev, download
 * - Keyboard shortcuts: / to focus search, Esc to close, ←/→ to navigate
 * - Persists favorites & last-selected category in localStorage
 *
 * How to plug in your images:
 * 1) Replace the SAMPLE_DATA array with your items (title, category, src, tags)
 * 2) If hosting files locally, place images in /public/guidelines/* and use
 *    src: "/guidelines/your-file.jpg" (Next.js will serve from /public)
 * 3) Optional: add pages or sections per guideline if you split long docs
 *
 * You can drop this in a Next.js App Router page (e.g. app/page.tsx)
 * or use it in CRA/Vite with Tailwind. Tailwind classes are used for styling.
 */
...
export type GuidelineItem = {
  id: string;
  title: string;
  category: string; // e.g., "Airway", "Pediatrics", "Cardiac"
...
const SAMPLE_DATA: GuidelineItem[] = [
  {
    ...
    title: "Circulation Cognitive Aide – TXA, Pelvic Binder, Fluids & Vasopressors",
...
// ----------------------- Main Component --------------------
export default function GuidelinesLibrary({ items = SAMPLE_DATA }: { items?: GuidelineItem[] }) {
  ...
  const onMouseMove: React.MouseEventHandler<HTMLDivElement> = (e) => {
    ...
    setPanning({ x: e.clientX - dragOrigin.current.x, y: e.clientY - dragOrigin.current.y });
  };
  ...
  const onTouchMove: React.TouchEventHandler<HTMLDivElement> = (e) => {
    if (e.touches.length === 1 && touchOrigin.current) {
      const t = e.touches[0];
      setPanning({ x: t.clientX - touchOrigin.current.x, y: t.clientY - touchOrigin.current.y });
    }
  };
  ...
  return (
    <div className="min-h-screen bg-neutral-950 text-neutral-100">
        ...
        <div className="mx-auto max-w-6xl px-4 py-3 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-neutral-400 -mt-0.5">Search · Filter · View · Download</p>
            </div>
          </div>
      ...
      <main className="mx-auto max-w-6xl px-4 py-6">
        {filtered.length === 0 ? (
          <div className="text-neutral-400 text-sm">No results. Try a different search or category.</div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {filtered.map((item) => {
              const fav = favorites.includes(item.id);
              return (
                <article key={item.id} className="group rounded-2xl border border-neutral-800 overflow-hidden bg-neutral-900">
                  <button
                    onClick={() => open(item)}
                  ...
                  <div className="p-3 flex items-start justify-between gap-2">
                    ...
                    <div className="flex items-center gap-2">
                      {item.note && (
                        <span className="text-[10px] text-neutral-400 hidden sm:inline">{item.note}</span>
                      )}
                      <button
                        onClick={() => toggleFav(item.id)}
                        className={cls(
                          "rounded-lg border px-2 py-1 text-xs transition",
                          fav ? "border-emerald-400 text-emerald-300" : "border-neutral-700 text-neutral-300 hover:border-neutral-500"
                        )}
                        aria-pressed={fav}
      ...
      {active && (
        <div
          ...
          aria-modal="true"
        >
          <button className="absolute inset-0 cursor-zoom-out" onClick={() => setActive(null)} aria-label="Close overlay" />
          ...
          <div className="relative z-10 w-full max-w-6xl">
            ...
            <div className="flex flex-wrap items-center justify-between gap-2 rounded-t-2xl bg-neutral-900/90 border border-b-0 border-neutral-800 px-3 py-2">
              <div className="flex items-center gap-2 text-sm">
                <strong className="truncate max-w-[40vw] sm:max-w-[24rem]">{active.title}</strong>
                <span className="text-neutral-400 hidden sm:inline">· {active.category}</span>
              </div>
              <div className="flex items-center gap-2 text-xs">
                <button onClick={() => nav(-1)} className="rounded-lg border border-neutral-700 px-2 py-1">← Prev</button>
                <button onClick={() => nav(1)} className="rounded-lg border border-neutral-700 px-2 py-1">Next →</button>
                <span className="mx-2 hidden sm:inline text-neutral-500">|</span>
                <button onClick={() => setZoom((z) => Math.max(0.5, z - 0.25))} className="rounded-lg border border-neutral-700 px-2 py-1">−</button>
                <span className="min-w-10 text-center">{Math.round(zoom * 100)}%</span>
                <button onClick={() => setZoom((z) => Math.min(8, z + 0.25))} className="rounded-lg border border-neutral-700 px-2 py-1">+</button>
                <button onClick={() => { setZoom(1); setPanning({ x: 0, y: 0 }); setRotation(0); }} className="rounded-lg border border-neutral-700 px-2 py-1">Reset</button>
                <button onClick={() => setRotation((r) => (r + 90) % 360)} className="rounded-lg border border-neutral-700 px-2 py-1">Rotate</button>
                <button onClick={() => toggleFav(active.id)} className="rounded-lg border border-neutral-700 px-2 py-1">
                  {favorites.includes(active.id) ? "★ Unfav" : "☆ Fav"}
                </button>
                <button onClick={downloadActive} className="rounded-lg border border-neutral-700 px-2 py-1">Download</button>
                <button onClick={() => setActive(null)} className="rounded-lg border border-neutral-700 px-2 py-1">Close ✕</button>
              </div>
            </div>
      ...
      <footer className="mx-auto max-w-6xl px-4 py-10 text-xs text-neutral-500">
        ...
        <ul className="list-disc pl-5 space-y-1">
          <li>Press <kbd className="rounded bg-neutral-800 px-1">/</kbd> to focus search. <kbd className="rounded bg-neutral-800 px-1">Esc</kbd> closes the viewer.</li>
          <li>Use mouse wheel (or pinch on mobile) to zoom; drag to pan; rotate in 90° steps.</li>
